...
ImapConnection.prototype.onLine = function (line) {
  ...
  if(this.state == States.NotAuthenticated) {
    ...
    switch(cmd) {
      ...
      case 'LSUB':
        if(args.length != 2) {
          this.send(tag, 'BAD', 'LSUB needs 2 arguments');
        } else {
          this.callPlugins('lsub', [this, args[0], args[1]], afterCommand.bind(this, tag));
        }
        return;
      case 'LIST':
        if(args.length != 2) {
      ...
      case 'STATUS':
        if(args.length != 2) {
          this.send(tag, 'BAD', 'STATUS needs 2 arguments');
        } else {
          this.callPlugins('status', [this, args[0], args[1]], afterCommand.bind(this, tag));
        }
        return;
      default:
        this.callPlugins('unknown_command', [this, cmd, args], afterCommand.bind(this, tag));
    ...
    switch(cmd) {
      case 'UID':
        this.callPlugins('uid', [this, args], afterCommand.bind(this, tag));
        return;
      case 'CLOSE':
